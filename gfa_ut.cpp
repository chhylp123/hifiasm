...
uint32_t iter_contain_g(R_to_U* rI, flex_asg_t *fg, uint32_t v0, asg64_v *b, asg64_v *st, float ou_rat)
{
...
void asg_arc_cut_contain(flex_asg_t *fg, asg64_v *in, asg64_v *in0, R_to_U* rI, float ou_rat)
{
    // fprintf(stderr, "+[M::%s]\n", __func__);
    ...
    for (v = 0; v < n_vtx; ++v) {
        ...
        cnt += iter_contain_g(rI, fg, v, b, b0, ou_rat);
        // if((v>>1) == 12321 || (v>>1) == 12334) {
        //     fprintf(stderr, "*[M::%s] v>>1::%u(%c), del::%u, contain::%u, fg->n::%u, fg->need_srt::%u\n", 
        //     __func__, (v>>1), "+-"[v&1], fg->g->seq[v>>1].del, is_contain_r((*rI), (v>>1)), (uint32_t)fg->n, (uint32_t)fg->need_srt);
        // }
...
double ou_drop_rate, int64_t max_tip, int64_t gap_fuzz, bub_label_t *b_mask_t, int32_t is_ou, int32_t is_trio, uint32_t ou_thres, char *o_file)
{
    ...
    if(is_ou) asg_arc_cut_contain(fg, &bu, &ba, rI, -1);
    if(!is_ou) asg_iterative_semi_circ(sg, src, &bu, max_tip, 1);
...
static void gen_scaffold_id(void *data, long i, int tid) // callback for kt_for()
{
    ...
    if(m0 && m1 && id != ((uint32_t)-1)) {
        // if((((v>>1) == 235) && ((w>>1) == 153)) || (((v>>1) == 153) && ((w>>1) == 235))) {
        //     fprintf(stderr, "[M::%s] id::%u, v>>1::%u(%c), w>>1::%u(%c)\n", 
        //     __func__, id, v>>1, "+-"[v&1], w>>1, "+-"[w&1]);
        // }
        z->ulid = (id<<1)|rev;
        z->raw_sof = m0->qe;
        z->raw_eof = m1->qs;
...
int32_t gen_spec_rc_edge(asg_t *rg, ug_opt_t *uopt, uint32_t v, uint32_t w, asg_arc_t *t)
{
...
{
    uint64_t uol, dif; int32_t r; asg_arc_t t, *p;
    uol = psa->raw_sof - psa->raw_eof;
    assert(uol >= (uint64_t)uopt->min_ovlp);
    r = gen_spec_rc_edge(ng, uopt, vx, wx, &t);
    if(r >= 0) {
    ...
    if(r >= 0) {
        ...
        r = gen_spec_rc_edge(ng, uopt, wx^1, vx^1, &t);
...
ul_renew_t *ropt)
{
    ...
    destory_bubbles(uidx->bub); free(uidx->bub); ma_ug_destroy(init_ug); 
